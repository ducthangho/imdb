...
inline
future<> kj_output_stream<CharType>::write(const char_type* buf) {
    return write(buf, strlen(buf));
}

template<typename CharType>
inline
future<> kj_output_stream<CharType>::write(const sstring& s) {
    return write(s.c_str(), s.size());
}

template<typename CharType>
future<> kj_output_stream<CharType>::write(scattered_message<CharType> msg) {
    return write(std::move(msg).release());
}

template<typename CharType>
future<> kj_output_stream<CharType>::write(net::packet p) {
...
kj_input_stream<CharType>::read_exactly_part(size_t n, tmp_buf out, size_t completed) {
    if (available()) {
...
template <typename CharType>
future<temporary_buffer<CharType>>
kj_input_stream<CharType>::read_exactly(size_t n) {
    if (_buf.size() == n) {
...
future<>
kj_input_stream<CharType>::consume(Consumer& consumer) {
    if (_buf.empty() && !_eof) {
...
template <typename CharType>
future<>
kj_output_stream<CharType>::split_and_put(temporary_buffer<CharType> buf) {
    assert(_end == 0);
...
template <typename CharType>
future<>
kj_output_stream<CharType>::write(const char_type* buf, size_t n) {
...
template <typename CharType>
future<>
kj_output_stream<CharType>::flush() {
    if (!_end) {
        return make_ready_future<>();
    }
    _buf.trim(_end);
