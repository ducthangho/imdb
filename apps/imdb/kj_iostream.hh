...
class kj_data_source_impl {
public:
    virtual ~kj_data_source_impl() {}
    virtual future<temporary_buffer<char>> get() = 0;
};

class kj_data_source {
    std::unique_ptr<kj_data_source_impl> _dsi;
protected:
    kj_data_source_impl* impl() const { return _dsi.get(); }
public:
    kj_data_source() = default;
    explicit kj_data_source(std::unique_ptr<kj_data_source_impl> dsi) : _dsi(std::move(dsi)) {}
    kj_data_source(kj_data_source&& x) = default;
    kj_data_source& operator=(kj_data_source&& x) = default;
    future<temporary_buffer<char>> get() { return _dsi->get(); }
};

class kj_data_sink_impl {
public:
    virtual ~kj_data_sink_impl() {}
...
class kj_data_sink {
    std::unique_ptr<kj_data_sink_impl> _dsi;
public:
    kj_data_sink() = default;
    explicit kj_data_sink(std::unique_ptr<kj_data_sink_impl> dsi) : _dsi(std::move(dsi)) {}
    kj_data_sink(kj_data_sink&& x) = default;
    kj_data_sink& operator=(kj_data_sink&& x) = default;
...
public:
    ...
    kj_input_stream() = default;
    explicit kj_input_stream(data_source fd) : _fd(std::move(fd)), _buf(0) {}
    kj_input_stream(kj_input_stream&&) = default;
    kj_input_stream& operator=(kj_input_stream&&) = default;
...
public:
    using char_type = CharType;
    kj_output_stream() = default;
    kj_output_stream(kj_data_sink fd, size_t size, bool trim_to_size = false)
        : _fd(std::move(fd)), _size(size), _trim_to_size(trim_to_size) {}
    kj_output_stream(kj_output_stream&&) = default;
    kj_output_stream& operator=(kj_output_stream&&) = default;
